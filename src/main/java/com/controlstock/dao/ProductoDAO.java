...
public class ProductoDAO {
    ...
    public Producto buscarPorId(int id) throws SQLException {
        try (Connection c = Database.getConnection(); PreparedStatement p = c.prepareStatement("SELECT * FROM productos WHERE id=?")) {
            ...
            try (ResultSet x = p.executeQuery()) {
                return x.next() ? new Producto(x.getInt("id"), x.getString("nombre"), x.getDouble("precio"), x.getInt("stock")) : null;
            }
        }
    }
    ...
    public int guardar(Producto v) throws SQLException {
        try (Connection c = Database.getConnection(); PreparedStatement p = c.prepareStatement("INSERT INTO productos(nombre,precio,stock) VALUES(?,?,?)", Statement.RETURN_GENERATED_KEYS)) {
            ...
            p.setDouble(2, v.getPrecio());
            p.setInt(3, v.getStock());
            p.executeUpdate();
    ...
    public void eliminar(int id) throws SQLException {
        try (Connection c = Database.getConnection(); PreparedStatement p = c.prepareStatement("DELETE FROM productos WHERE id=?")) {
            p.setInt(1, id);
            p.executeUpdate();
        }
    }
