...
public class ClienteDAO {
    ...
    public Cliente buscarPorId(int id) throws SQLException {
        try (Connection c = Database.getConnection(); PreparedStatement p = c.prepareStatement("SELECT * FROM clientes WHERE id=?")) {
            ...
            try (ResultSet x = p.executeQuery()) {
                return x.next() ? new Cliente(x.getInt("id"), x.getString("nombre"), x.getString("correo"), x.getString("telefono")) : null;
            }
        }
    }
    ...
    public int guardar(Cliente v) throws SQLException {
        try (Connection c = Database.getConnection(); PreparedStatement p = c.prepareStatement("INSERT INTO clientes(nombre,correo,telefono) VALUES(?,?,?)", Statement.RETURN_GENERATED_KEYS)) {
            ...
            p.setString(2, v.getCorreo());
            p.setString(3, v.getTelefono());
            p.executeUpdate();
    ...
    public void eliminar(int id) throws SQLException {
        try (Connection c = Database.getConnection(); PreparedStatement p = c.prepareStatement("DELETE FROM clientes WHERE id=?")) {
            p.setInt(1, id);
            p.executeUpdate();
        }
    }
