...
public class ClienteApiServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            resp.getWriter().print(gson.toJson(id == null ? dao.listar() : dao.buscarPorId(Integer.parseInt(id))));
        } catch (Exception e) {
            error(resp, e, 500);
        }
    }

    // Recibe un cliente en formato JSON, lo guarda y devuelve el registro creado.
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        json(resp);
        try {
            Cliente c = gson.fromJson(req.getReader(), Cliente.class);
            c.setId(dao.guardar(c));
            resp.setStatus(201);
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            if (idParam == null) {
                error(resp, new Exception("Debe indicar el parámetro id"), 400);
                return;
            }
            Cliente existente = dao.buscarPorId(Integer.parseInt(idParam));
            if (existente == null) {
                error(resp, new Exception("Cliente no encontrado"), 404);
                return;
            }
    ...
    // Define que las respuestas de la API se entregan como JSON en UTF-8.
    private void json(HttpServletResponse r) {
        r.setContentType("application/json;charset=UTF-8");
    }

    // Devuelve los errores de la API con su código HTTP y mensaje en JSON.
    private void error(HttpServletResponse r, Exception e, int s) throws IOException {
        r.setStatus(s);
        r.getWriter().print(gson.toJson(java.util.Collections.singletonMap("error", e.getMessage())));
    }
}
