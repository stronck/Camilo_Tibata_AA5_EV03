...
public class ProductoApiServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest q, HttpServletResponse r) throws IOException {
        ...
        try {
            ...
            r.getWriter().print(gson.toJson(id == null ? dao.listar() : dao.buscarPorId(Integer.parseInt(id))));
        } catch (Exception e) {
            r.setStatus(500);
    ...
    protected void doPost(HttpServletRequest q, HttpServletResponse r) throws IOException {
        r.setContentType("application/json;charset=UTF-8");
        try {
            Producto p = gson.fromJson(q.getReader(), Producto.class);
            p.setId(dao.guardar(p));
            r.setStatus(201);
            r.getWriter().print(gson.toJson(p));
        } catch (Exception e) {
            r.setStatus(400);
            r.getWriter().print(gson.toJson(java.util.Collections.singletonMap("error", e.getMessage())));
        }
    ...
    protected void doDelete(HttpServletRequest q, HttpServletResponse r) throws IOException {
        r.setContentType("application/json;charset=UTF-8");
        try {
            dao.eliminar(Integer.parseInt(q.getParameter("id")));
            r.getWriter().print("{\"ok\":true}");
        } catch (Exception e) {
            r.setStatus(400);
            r.getWriter().print(gson.toJson(java.util.Collections.singletonMap("error", e.getMessage())));
        }
