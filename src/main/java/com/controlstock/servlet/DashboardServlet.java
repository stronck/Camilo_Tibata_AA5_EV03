...
public class DashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest q, HttpServletResponse r) throws ServletException, IOException {
        if (q.getSession().getAttribute("usuario") == null) {
            r.sendRedirect(q.getContextPath() + "/login.jsp");
            return;
        }
        try {
            q.setAttribute("clientes", new ClienteDAO().listar());
            q.setAttribute("productos", new ProductoDAO().listar());
            q.setAttribute("ventas", new VentaDAO().listar());
            q.getRequestDispatcher("/views/dashboard.jsp").forward(q, r);
        } catch (Exception e) {
